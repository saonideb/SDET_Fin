...
public partial class Individual_Users_Edit_Form : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
            ...
            dsDetail = CC.Getdata("select ROW_NUMBER() OVER(ORDER BY ID ASC) as RowNumber,ItemDescription as txt_item_description,Quantity as txt_item_quantity,ApproxTotalCost as txt_item_cost from tbl_Individual_Consumables_form_details where Approval_No='" + Request.QueryString["Approval_No"].ToString() + "'");
            if (dsDetail != null && dsDetail.Tables[0].Rows.Count > 0)
            {
                ...
                ViewState["CurrentTable"] = dsDetail.Tables[0];

                grd_Travel_details.DataSource = dsDetail.Tables[0];
            ...
            string str8 = "SELECT COUNT(*) FROM tbl_Individual_Consumables_form WHERE Financial_Year = '" + financial_year.ToString() + "' AND Department_Id = '" + department_division_id.ToString() + "' AND   Head = '" + budget_head_for_balance.ToString() + "' AND Sub_Head = '" + budget_sub_head_for_balance.ToString() + "' AND Approval_Status ='Approved' ";
            ...
            else if (institute_approval_record_count > 0)
            {
                ...
                logincommand10.CommandText = "SELECT CASE WHEN SUM(cast(Estimated_Amount as decimal(10,2))) is null then '0' ELSE SUM(cast(Estimated_Amount as decimal(10,2))) END as Estimated_Amount FROM tbl_Individual_Consumables_form WHERE Financial_Year = '" + financial_year.ToString() + "' AND Department_Id = '" + department_division_id.ToString() + "' AND  Head = '" + budget_head_for_balance.ToString() + "' AND Sub_Head = '" + budget_sub_head_for_balance.ToString() + "'  AND Approval_Status IN ('Approved') and Bill_Amount_Submit_Status = 'Pending'";//AND Dean_Approval_Status IN ('Approved') Bill_Amount_Submit_Status = 'Pending'
    ...
    protected void btn_submit_Click(object sender, EventArgs e)
    {
        if (Session["captcha"].ToString() == txt_captcha_code.Text)
        {
            ...
            string reference_No = newGuid.ToString();
            Session["Form_A_Reference_No"] = reference_No;

            decimal TotalCost = 0;
            string ApprvalNo = Request.QueryString["Approval_No"].ToString();
            ...
            if (ds21 != null && ds21.Tables[0].Rows.Count > 0)
            {
                ...
                CC.Getdata("update tbl_Individual_Consumables_form set  " + y1 + " Bill_Amount_Submit_Status='Pending',Approval_Status='Pending',All_Total_Cost='" + TotalCost + "',Estimated_Amount='" + TotalCost + "' where Approval_No='" + Request.QueryString["Approval_No"].ToString() + "'");
            }


            DataSet dsDetail = new DataSet();
            dsDetail = CC.Getdata("select * from tbl_Individual_Consumables_form_details where Approval_No='" + Request.QueryString["Approval_No"].ToString() + "'");
            if (dsDetail != null && dsDetail.Tables[0].Rows.Count > 0)
            {
                CC.Getdata("delete from tbl_Individual_Consumables_form_Details where Approval_No='" + Request.QueryString["Approval_No"].ToString() + "'");
            }
            ...
            for (int x = 0; x < grd_Travel_details.Rows.Count; x++)
            {
                ...
                + " values ('" + Session["Form_A_Reference_No"].ToString() + "','" + txt_item_description.Text.Replace("'", "''") + "','" + txt_item_quantity.Text.Replace("'", "''") + "','" + txt_item_cost.Text.Replace("'", "''") + "','" + TotalCost + "','" + ApprvalNo + "','" + Session["Department_Divison_Id"].ToString() + "','" + Session["Form_A_Entry_Financial_Year"].ToString() + "')");
