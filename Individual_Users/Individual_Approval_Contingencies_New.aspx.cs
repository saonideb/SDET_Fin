...
public partial class Individual_Users_Individual_Approval_Contingencies_New : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            else if (institute_approval_record_count > 0)
            {
                ...
                // ' logincommand10.CommandText = "SELECT CASE WHEN SUM(cast(Estimated_Amount as decimal(10,2))) is null then '0' ELSE SUM(cast(Estimated_Amount as decimal(10,2))) END as Estimated_Amount FROM Institute_Approval_Master WHERE Financial_Year = '" + financial_year.ToString() + "' AND Department_Id = '" + department_division_id.ToString() + "' AND Main_Head = '" + budget_main_head_for_balance.ToString() + "' AND Head = '" + budget_head_for_balance.ToString() + "' AND Sub_Head = '" + budget_sub_head_for_balance.ToString() + "' AND Dean_Approval_Status IN ('Approved') AND Bill_Amount_Submit_Status = 'Pending'"
    ...
    //        CC.Getdata("insert into tbl_Individual_Contingencies_form(UserName,BudgetHead,ProjectTitle,BudgetHead_BalanceAmount,Justification,Added_By,AddedByName,Added_On,IndividualIP_Address,All_Total_Cost,Department)"
    ...
    //            CC.Getdata("insert into tbl_Individual_Contingencies_form_Details(Added_By,ItemDescription,Quantity,ApproxTotalCost,All_Total_Cost)"
            ...
            if (TotalCost <= Balance11)
            {
                ...
                if (TotalCost <= 10000)
                {
                    CC.Getdata("insert into tbl_Individual_Consumables_form(UserName,Head,ProjectTitle,BudgetHead_BalanceAmount,Justification,Added_By,AddedByName,Added_On,IndividualIP_Address,All_Total_Cost,Estimated_Amount, Department,Approval_No,Financial_Year,Sub_Head,Sequence_no,Approval_Status,Department_ID,HOD_Approval_Status,SRCD_Dean_Approval_Status,Dean_Approval_Status,Director_Approval_Status,Bill_Amount_Submit_Status,Bill_Amount)"
                                   ...
                                   + " '" + lbl_budget_head_balance_amount.Text.Replace("'", "''") + "','" + txt_justification.Text.Replace("'", "''") + "','" + Request.QueryString["Session_Id"].ToString() + "',"
                                   + "'" + Session["Email"].ToString() + "',getdate(),'" + Session["User_IP_Address"].ToString() + "','" + TotalCost + "','" + TotalCost + "','" + lbl_department_name.Text + "','" + ApprvalNo + "','" + Session["Form_A_Entry_Financial_Year"].ToString() + "','" + Session["Form_A_Entry_Sub_Head"].ToString() + "','" + Last_Sequence_No + "','Approved','" + Session["Department_Divison_Id"].ToString() + "','Approval Not Required','Approval Not Required','Approval Not Required','Approval Not Required','Pending','0')");
                }
                else
                {
                    CC.Getdata("insert into tbl_Individual_Consumables_form(UserName,Head,ProjectTitle,BudgetHead_BalanceAmount,Justification,Added_By,AddedByName,Added_On,IndividualIP_Address,All_Total_Cost,Estimated_Amount, Department,Approval_No,Financial_Year,Sub_Head,Sequence_no,Approval_Status,Department_ID,HOD_Approval_Status,SRCD_Dean_Approval_Status,Dean_Approval_Status,Director_Approval_Status,Bill_Amount_Submit_Status,Bill_Amount)"
                ...
                SqlCommand logincommand17 = new SqlCommand();
                logincommand17.CommandText = "SELECT Username FROM HOD_Login_Details where Department_Name='"+lbl_department_name.Text+"'";
                logincommand17.Connection = con;
                con.Open();
                objreader17 = logincommand17.ExecuteReader();
                if (objreader17.Read())
                {
                    hod_email_id = objreader17["Username"].ToString();
                }
                con.Close();
                //Get Get Faculty Details
                string Mail_Name = ""; string Mail_Dept = ""; string Mail_ProjectTitle = "";
                SqlDataReader objreader18;
                SqlCommand logincommand18 = new SqlCommand();
                logincommand18.CommandText = "SELECT *FROM tbl_Individual_Consumables_form where Approval_No='" + ApprvalNo+ "'";
                logincommand18.Connection = con;
                con.Open();
                objreader18 = logincommand18.ExecuteReader();
                if (objreader18.Read())
                {
                    Mail_Name = objreader18["UserName"].ToString();
                    Mail_Dept = objreader18["Department"].ToString();
                    Mail_ProjectTitle = objreader18["ProjectTitle"].ToString();
                ...
                mail_to = hod_email_id.ToString();
                //mail_to = "atul@pilani.bits-pilani.ac.in";


                from_address = "deanadmin.approvals@pilani.bits-pilani.ac.in";
                subject = "Project Approval Request (Approval No.:" + ApprvalNo.ToUpper() + ")";
               // string approval_status = "Pending";
                ...
                try
                {
                    mail.Body = ("<html><body><p><b>Dear "
                                + (user_name.ToString() + ",</b></p><br>"));
                    mail.Body = (mail.Body + "One of the approval request requires your approval.The approval details are as follows:<br><br>");
                    mail.Body = (mail.Body + ("<strong>Approval No.:  </strong>" + (ApprvalNo.ToUpper() + "<br><br>")));
                    mail.Body = (mail.Body + ("<strong>Name of the User:  </strong>"
                                + (Mail_Name.ToString() + "<br><br>")));

                    mail.Body = (mail.Body + ("<strong>Department:  </strong>"
                                + (Mail_Dept.ToString() + "<br><br>")));

                    mail.Body = (mail.Body + ("<strong>Project Title:  </strong>"
                               + (Mail_ProjectTitle.ToString() + "<br><br>")));


                    //mail.Body = (mail.Body + ("<strong>Approval Date :  </strong>"
                             //   + (lbl_current_date_p.Text.ToString() + "<br><br>")));
                   
                    mail.Body = (mail.Body + ("<strong>Request Date & Time:  </strong>"+ (Session["Mail_Processed_On"].ToString() + "<br><br>")));
                    mail.Body = (mail.Body + "<strong>Please visit BITS Approval System Portal (https://ipcservices.bits-pilani.ac.in/BASHOD/SignIn.aspx) to view the approval status.</strong>");
