...
public partial class Individual_Users_View_Individual_Approval_Status_Step2 : System.Web.UI.Page
{
    ...
    private void BindGrid()
    {
        ...
        ds = Gd.Getdata("SELECT * FROM tbl_Individual_Consumables_form WHERE Financial_Year = '" + current_financial_year.ToString() + "' "
                      + " AND Department_Id = '" + department_division_id.ToString() + "'  AND   AddedByName = '" + Session["Email"].ToString() + "' order by Consumable_ID desc");//and Approval_Status!='Not Approved'
